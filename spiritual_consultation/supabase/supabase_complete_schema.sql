-- ═════════════════════════════════════════════════════════════════════
-- 🌿 BioPara Spiritual — قاعدة البيانات الكاملة والنهائية المخصصة (Unified Scheme)
-- ═════════════════════════════════════════════════════════════════════
-- قم بنسخ هذا الملف بالكامل وتشغيله في محرر SQL الخاص بـ Supabase (SQL Editor)
-- ═════════════════════════════════════════════════════════════════════

-- ─── 0. تنظيف قاعدة البيانات (Reset) لتفادي أي أخطاء تكرار أو تعارض ───
...
-- هذا الجدول يمثل الملف الشخصي لكل مستخدم (مريض أو مسؤول) وهو كافٍ وموحد
...
DECLARE
  v_is_admin BOOLEAN;
BEGIN
  -- نستعلم مباشرة من جدول الحماية الداخلي auth.users لمنع التكرار اللانهائي
...
CREATE TABLE IF NOT EXISTS public.messages (
  ...
  sender_id       TEXT NOT NULL, -- TEXT to support both user UUIDs and 'ai_agent'
  content         TEXT,
  message_type    TEXT NOT NULL DEFAULT 'text', -- text | image | audio | video | file | ai
  file_url        TEXT,
  is_read         BOOLEAN NOT NULL DEFAULT FALSE,
  is_ai           BOOLEAN NOT NULL DEFAULT FALSE,
  status          TEXT NOT NULL DEFAULT 'sent', -- sent | delivered | read
  metadata        JSONB DEFAULT '{}'::jsonb, -- JSONB to store arbitrary metadata (replies, call info)
  created_at      TIMESTAMPTZ NOT NULL DEFAULT NOW()
);
...
-- تريجر تحديث المحادثة تلقائياً عند إضافة رسالة جديدة
...
-- ─── 8. جدول المواعيد والاستشارات (Appointments) ──────────────────────
-- هذا الجدول الموحد يمثل جميع جلسات الحجز للمريض (سواء محادثة، فيديو، أو صوت)
...
-- ─── 9. جدول قائمة الأماني (Wishlists) ─────────────────────────────
...
-- ─── 10. جدول تقييمات المنتجات (Reviews) ──────────────────────────────
...
-- ─── 12. تفعيل RLS وسياسات حماية البيانات الجديدة والآمنة ───
...
CREATE POLICY "appointments_admin_all" ON public.appointments FOR ALL USING (public.is_admin());
...
-- ─── 13. إنشاء وتجهيز Buckets التخزين (Storage Buckets) ───
...
-- ─── 14. تعيين الحساب التجريبي كمدير وتحديث كاش الـ API ───
...
-- مزامنة واسترجاع الملفات الشخصية لجميع المستخدمين المسجلين مسبقاً في auth.users
-- (هذا يمنع تعطل الجلسات النشطة للمستخدمين بعد عملية إعادة تهيئة قاعدة البيانات)
INSERT INTO public.profiles (id, full_name, phone)
SELECT id, COALESCE(raw_user_meta_data->>'full_name', 'مستخدم BioPara'), phone
FROM auth.users
ON CONFLICT (id) DO NOTHING;

-- إعادة تحميل كاش Supabase API

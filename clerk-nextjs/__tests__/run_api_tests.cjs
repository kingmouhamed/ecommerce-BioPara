...
const testCases = [
  ...
  },
  {
    ...
    method: 'POST',
    body: {},
    expectedStatus: [400, 401, 307] // accepts 400 validation if auth isn't hit first, or 401/307 if blocked
  },
  {
    name: 'DELETE /api/cart (unauthorized)',
    url: '/api/cart',
    method: 'DELETE',
    expectedStatus: [401, 307, 405] // accepts 405 Method Not Allowed if not defined, or 401/307 if blocked
  },
  {
...
async function runTests() {
  ...
  for (const tc of testCases) {
    ...
    if (res.passed) {
      passedCount++;
      console.log(`✅ PASS: ${res.name} (Expected: ${JSON.stringify(res.expectedStatus)}, Got: ${res.actualStatus})`);
    } else {
      failedCount++;
      console.log(`❌ FAIL: ${res.name} (Expected: ${JSON.stringify(res.expectedStatus)}, Got: ${res.actualStatus})`);
